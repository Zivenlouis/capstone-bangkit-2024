...
import com.capstoneproject.auxilium.databinding.FragmentQuest4Binding

class Quest4Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val question4Response = sharedViewModel.getQuestion4Response()
        setSelectedRadioButton(question4Response)

        binding.radioGroup.setOnCheckedChangeListener { group, checkedId ->
            val radioButton = group.findViewById<RadioButton>(checkedId)
            radioButton?.let {
                val selectedOption = mapRadioButtonTextToInt(it.text.toString())
                sharedViewModel.setQuestion4Response(selectedOption)
            }
        }

        binding.nextButton.setOnClickListener {
            findNavController().navigate(R.id.Fragment4toFragment5)
        }

        binding.backButton.setOnClickListener {
            findNavController().navigate(R.id.Fragment4toFragment3)
        }
    }
