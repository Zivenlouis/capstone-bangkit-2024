...
# def combine_collaborative_model():
...
#     X = tf.keras.layers.Dense(units=32, activation=None, kernel_initializer='random_normal', name='X')(X) # From the first hyperparameter tuning result
#     activation = 'sigmoid'  # From the first hyperparameter tuning result
#     X = tf.keras.layers.Activation(activation)(X) # From the first hyperparameter tuning result
#     X = tf.keras.layers.Dense(units=32, activation=activation, kernel_initializer='random_normal', name='X2')(X) # From the first hyperparameter tuning result
...
#     rating_input = tf.keras.Input(shape=(PHONE_COUNT,), dtype=tf.int32, name='rating_input')
#     X = tf.keras.layers.Concatenate(name='concatenated_inputs')([rating_input, click_input])
#     X = tf.keras.layers.Dense(units=32, activation=None, kernel_initializer='random_normal', name='X')(X) # From the second hyperparameter tuning result
#     X = tf.keras.layers.Activation('relu')(X) # From the second hyperparameter tuning result
#     X = tf.keras.layers.Dense(units=288, activation='relu', kernel_initializer='random_normal', name='X2')(X) # From the second hyperparameter tuning result
...
def combine_collaborative_model():
    ...
    rating_input = tf.keras.Input(shape=(PHONE_COUNT,), dtype=tf.int32, name='rating_input')    
    X = tf.keras.layers.Concatenate(name='concatenated_inputs')([rating_input, click_input])
    X = tf.keras.layers.Dense(units=128, activation='sigmoid', kernel_initializer='random_normal', name='X')(X) # From the third hyperparameter tuning result
    X = tf.keras.layers.Dense(units=352, activation='sigmoid', kernel_initializer='random_normal', name='X2')(X) # From the third hyperparameter tuning result
    output_click = tf.keras.layers.Dense(PHONE_COUNT, activation='sigmoid', name='output')(X)
    model = tf.keras.Model(inputs=[rating_input, click_input], outputs=output_click, name='collaborative_model')    
    return model

model = combine_collaborative_model()
# model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0069108517839247645), loss=tf.keras.losses.MeanSquaredError()) # From the first hyperparameter tuning result
# model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.006693994895295038), loss=tf.keras.losses.MeanSquaredError()) # From the second hyperparameter tuning result
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0014618661689036517), loss=tf.keras.losses.MeanSquaredError()) # From the third hyperparameter tuning result
...
model.save('collaborative_filtering_1.h5')
